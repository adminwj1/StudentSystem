        ...
        try:
            ...
            if reslist:
                sql_a = "update info set name = '%s',gender = '%s',age = '%s', cardid = '%s', date = '%s', address = '%s', department = '%s', major = '%s', class = '%s', phone = '%s', mail = '%s', QQ = '%s' where id = '%s'" % \
                        (self.input_name.get(), self.gender_a.get(), self.input_age.get(), self.input_cardid.get(), self.input_date.get(), self.input_date.get(), self.input_department.get(),
                        self.input_major.get(), self.input_Class, self.input_phone.get(), self.input_mail.get(), self.input_QQ.get(), self.input_id.get())
            ...
            db.rollback()
            cursor.close()
            messagebox.showinfo(title='修改', message='修加失败！')
        finally:
            db.close()
