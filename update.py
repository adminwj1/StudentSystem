...
class Update(object):
    ...
    def SQL(self):
        ...
        cursor = db.cursor()
        try:
            sql = "update info set name = '%s',gender = '%s',age = '%s', cardid = '%s', date = '%s', address = '%s', department = '%s', major = '%s', class = '%s', phone = '%s', mail = '%s', QQ = '%s' where id = '%s'" % \
                (name, gender, age, cardid, date, address, department, major, Class, phone, mail, QQ, id)
            cursor.execute(sql)
            db.commit()  # 提交事务
            messagebox.showinfo(title='添加', message='添加成功！')
        except:
            db.rollback()
            cursor.close()
